...
var DEBUG = true
var TARGET_SIZE = {width:640,height:960}
$.level = 1;

var toolVersion = "psdAnimToolv0.5";
var scriptName = "BannerGenator";
...
function removeMultSpaceInStr(str)
{
	str = str.replace(/¡¡/g, " ");
	while(str.indexOf("  ")>=0)
	{
...
var printFile = new File(basePath + "/print.txt");
    printFile.encoding = "UTF8";
    printFile.open( "w", "TEXT", "????" );
    printFile.write("\uFEFF"); 

function print (logContent) {
    if (DEBUG == null || DEBUG == false)
        return
    printFile.write(logContent+"\r\n"); 
}
...
function exprotPlistParse () {
    for(var keyname in symbolLib)
    {
            ...
            var name = itemOriginOb["libName"];
                                    print("keyname==="+name)
                                    print("itemOriginOb==="+itemOriginOb.toString())
            ...
            // layerInfoOb["keyFrameCount"] = 1;
                                    var childLayer = {};
                           itemInfoOb["childLayer"]=childLayer;
                        
            var ins = itemOriginOb["children"]; 
            if(ins == null)
                                        continue;
            // childLayer["elementNum"] = ins.length;
            // var elementDict={};
            // childLayer["elementDict"] = elementDict;
            for(var h=0; h<ins.length; h++)
            {
                var elementInfoOb = {};
                var elementName = ""+h;
                childLayer[elementName]=elementInfoOb;
                checkOneInstance(elementInfoOb,ins[h]);
            }
    }
}

function exprotXMLParse () {
    for(var keyname in symbolLib)
    {
        var itemOriginOb = symbolLib[keyname];
        var itemInfoOb = {};
...
function getExprotCSDHead() {
    var docUpdateDate  =docFileObject.modified;
    var UpdateTime =  docUpdateDate.getTime() ;
    var finalXml="<GameProjectFile>\n";
    finalXml+="  <PropertyGroup Type=\"Layer\" Name=\""+fileNamePre+"\" ID=\""+UpdateTime+"\" Version=\"2.3.2.3\" \/>\n";
    finalXml+="  <Content ctype=\"GameProjectContent\">\n"
    finalXml+="    <Content>\n"
    finalXml+="      "+getFormatStr("Animation",{ Duration:"0",Speed:"1.0000"})
    finalXml+="      <ObjectData Name=\"Layer\" Tag=\"0\" ctype=\"GameLayerObjectData\">\n"
    finalXml+="        "+getFormatStr("Size",{ X:TARGET_SIZE.width,Y:TARGET_SIZE.height})
    finalXml+="          <Children>\n";
    return finalXml
}

function getExprotCSDEnd() {
    var finalXml="";
    finalXml+="        <\/Children>\n";
    finalXml+="      <\/ObjectData>\n"
    finalXml+="    <\/Content>\n"; 
    finalXml+="  <\/Content>\n"; 
    finalXml+="<\/GameProjectFile>\n"; 
    return finalXml
}

function getArributeStr (key,value) {
    if (value == null)
    {
        value = ""
    }
...
function getFormatStr (key,arributeTable) {
    // body...
    var retXmlContent = "    <"+key+" "
    for(var keyname in arributeTable)
    {
        retXmlContent+=getArributeStr(keyname,arributeTable[keyname])
    }
    retXmlContent += "\/>\n"
    return retXmlContent
}

function checkVar (cVar,defaultValue) {
    // body...
...
function getButtonXml(libItem,tab)
{
	
     var spaceStr = ""
     for(var tabi=1;tabi<=tab;tabi++)
     {
        spaceStr+="    "
     }

     var finalXml="";
	 if(libItem.type == "Button")
     {
         var ActionTag = "0"
         ...
         var TopMargin = "0"
         var BottomMargin = "0"
         var ctype = "ButtonObjectData"
         var sizeX = libItem.width
         var sizeY = libItem.height
         if (sizeX == null) sizeX = TARGET_SIZE.width
         if (sizeY == null) sizeY = TARGET_SIZE.height
         var PositionX = libItem.x
         var PositionY = libItem.y
         var Tag = libItem.depth
         print("libItem.libName== "+libItem.libName)
         print("sizeY== "+sizeY)
         ...
         var defaultScaleX =1 
         var defaultScaleY = 1
         var defaultScaleX =libItem.scaleX
         var defaultScaleY = libItem.scaleY
		 if(isNaN(defaultScaleX)) defaultScaleX=1.0
		 if(isNaN(defaultScaleY)) defaultScaleY=1.0
		 defaultScaleX=defaultScaleX*1.0
		 defaultScaleX=defaultScaleX*1.0
            var shortname = libItem.libName.split("_")[0]
         var Name = "Button_"+shortname

         finalXml+=getArributeStr("Name",Name) 
         finalXml+=getArributeStr("ActionTag",ActionTag)  
         finalXml+=getArributeStr("Tag",Tag)
         finalXml+=getArributeStr("IconVisible",IconVisible) 
         finalXml+=getArributeStr("LeftMargin",LeftMargin)  
         finalXml+=getArributeStr("RightMargin",RightMargin)
         finalXml+=getArributeStr("TopMargin",TopMargin)
         finalXml+=getArributeStr("BottomMargin",BottomMargin) 
         finalXml+=getArributeStr("TouchEnable","True")
         finalXml+=getArributeStr("BackColorAlpha","0")
         finalXml+=getArributeStr("ComboBoxIndex","1")
         finalXml+=getArributeStr("ColorAngle","90.0")
         finalXml+=getArributeStr("CallBackType","Click")
         finalXml+=getArributeStr("ButtonText","")
         finalXml+=getArributeStr("ctype",ctype)

         finalXml+= ">\n"
         finalXml+=spaceStr +  getFormatStr("Size",{ X:sizeX,Y:sizeY})
         finalXml+=spaceStr +  getFormatStr("AnchorPoint",{})
         finalXml+=spaceStr +  getFormatStr("Position",{X:PositionX,Y:PositionY})
         finalXml+=spaceStr +  getFormatStr("Scale",{ScaleX:defaultScaleX,ScaleY: defaultScaleY})
         finalXml+=spaceStr +  getFormatStr("CColor",{A:"255",R:"255",G:"255",B:"255"})
         finalXml+=spaceStr +  getFormatStr("PrePosition",{X:"0.0",Y:"0.0"})
         finalXml+=spaceStr +  getFormatStr("PreSize",{X:"0.0000", Y:"0.0000"})
		 
         finalXml+=spaceStr +  getFormatStr("TextColor",{A:"255",R:"150",G:"200",B:"255"})
         finalXml+=spaceStr +  getFormatStr("Default",{A:"255",R:"150",G:"200",B:"255"})
		 var imageName =fileNamePre+"\/"+libItem.children[0].children[0].sourceName
          var  childrenCount = 0;
           for(var i in libItem.children)
           {
               childrenCount = childrenCount +1;
           }
         if ( childrenCount ==1)
         {
             var imageName1 =fileNamePre+"\/"+libItem.children[0].children[0].sourceName
			finalXml+=spaceStr +  getFormatStr("NormalFileData",{Type:"Normal",Path:imageName1,Plist:""})
             }
         else if ( childrenCount ==2)
         {
             var imageName =fileNamePre+"\/"+libItem.children[0].children[0].sourceName
             finalXml+=spaceStr +  getFormatStr("PressedFileData",{Type:"Normal",Path:imageName,Plist:""})
             var imageName1 =fileNamePre+"\/"+libItem.children[1].children[0].sourceName
			finalXml+=spaceStr +  getFormatStr("NormalFileData",{Type:"Normal",Path:imageName1,Plist:""})
          }
      
         finalXml+=spaceStr +  getFormatStr("OutlineColor",{A:"255",R:"0",G:"0",B:"0"})
         finalXml+=spaceStr +  getFormatStr("ShadowColor",{A:"255",R:"0",G:"0",B:"0"})

         finalXml+=spaceStr +  getFormatStr("SingleColor",{A:"255",R:"150",G:"200",B:"255"})
         finalXml+=spaceStr +  getFormatStr("FirstColor",{A:"255",R:"150",G:"200",B:"255"})
         finalXml+=spaceStr +  getFormatStr("EndColor",{A:"255",R:"255",G:"255",B:"255"})
         finalXml+=spaceStr +  getFormatStr("ColorVector",{ScaleY:"1.000"})     
     }
    return finalXml;
	
}
function getChildRenXml (libItem,tab) {
     var spaceStr = ""
     for(var tabi=1;tabi<=tab;tabi++)
     {
        spaceStr+="    "
     ...
     else if(libItem.type == "SimpleSprite")
     {      
            var childrenCount = 0
            for(var i in libItem.children)
            {
            ...
            if (childrenCount == 1&&libItem.children[0].type == 'Bitmap')  //取bitmap
            {       
                     itemInfo = libItem.children[0];
                     var ActionTag = "0"
                     var IconVisible = "False"
                     var LeftMargin = "0"
                     var RightMargin = "0"
                     var TopMargin = "0"
                     var BottomMargin = "0"
                     var ctype = "ImageViewObjectData"
                     var sizeX = itemInfo.width
                     var sizeY = itemInfo.height
                     if (sizeX == null) sizeX = TARGET_SIZE.width
                     if (sizeY == null) sizeY = TARGET_SIZE.height
                     var PositionX = libItem.x
                     var PositionY =  libItem.y
                    ...
                    if (PositionY == null )  PositionY = 0
                     if (PositionX == null ) PositionX = 0
                     if (Tag == null) Tag = 0
                     var defaultScaleX =itemInfo.scaleX
                     var defaultScaleY = itemInfo.scaleY
                     if(isNaN(defaultScaleX)) defaultScaleX=1.0
                     if(isNaN(defaultScaleY)) defaultScaleY=1.0
                     defaultScaleX=defaultScaleX*1.0
                     defaultScaleX=defaultScaleX*1.0
                     var imageName =fileNamePre+"\/"+itemInfo.sourceName

                     var shortname = itemInfo.libName.split("_")[0]
                     var Name = "Image_"+shortname
                     ...
                     finalXml+= ">\n"
                     finalXml+=spaceStr +  getFormatStr("Size",{ X:sizeX,Y:sizeY})
                     finalXml+=spaceStr +  getFormatStr("AnchorPoint",{ScaleX:"0",ScaleY:"0"})
                     finalXml+=spaceStr +  getFormatStr("Position",{X:PositionX,Y:PositionY})
                     finalXml+=spaceStr +  getFormatStr("Scale",{ScaleX:defaultScaleX,ScaleY: defaultScaleY})
                     finalXml+=spaceStr +  getFormatStr("CColor",{A:"255",R:"255",G:"255",B:"255"})
                     finalXml+=spaceStr +  getFormatStr("PrePosition",{X:"0.0",Y:"0.0"})
                     finalXml+=spaceStr +  getFormatStr("PreSize",{X:"0.0000", Y:"0.0000"})
                     finalXml+=spaceStr +  getFormatStr("FileData",{Type:"Normal",Path:imageName,Plist:""})
                     finalXml+=spaceStr +  getFormatStr("BlendFunc",{Src:"770",Dst:"771"})     
            }
            else //作为panel
            {
                            var ActionTag = "0"
                             ...
                             defaultScaleX=defaultScaleX*1.0
                             defaultScaleX=defaultScaleX*1.0
                             var imageName = libItem.sourceName
                             var shortname = libItem.libName.split("_")[0]
                             var Name = "Panel_"+shortname
     ...
     else if(libItem.type == "Text")
     {
         ...
         var TopMargin = "0"
         var BottomMargin = "0"
         var FontSize = Math.floor(libItem.fontSize) 
         var FontName = libItem.fontName

         var LabelText = libItem.textOriginContext
         checkVar(LabelText,"text")
         var ctype = "TextObjectData"

         var ColorR = libItem.textRed
         var ColorG = libItem.textGrain
         var ColorB = libItem.textBlue
         var ShadowOffsetX = "0"
         var ShadowOffsetY = "0"
         var sizeX = libItem.width
         var sizeY = libItem.height
         if (sizeX == null) sizeX = TARGET_SIZE.height
         if (sizeY == null) sizeY = TARGET_SIZE.width
        ...
        var HorizontalAlignmentType
         if (libItem.alignmentType == 33)
         {
            HorizontalAlignmentType = "HT_Center"
         }else if(libItem.alignmentType == 32)
         {
            HorizontalAlignmentType = "HT_Right"
         }
         else 
         {
             HorizontalAlignmentType = "HT_Left"
         }
         var VerticalAlignmentType="VT_Top"
		 ...
		 defaultScaleX=defaultScaleX*1.0
		 defaultScaleX=defaultScaleX*1.0
         var shortname = libItem.instanceName.split("_")[0]
         //var shortname = libItem.libName.split("_")[0]
         var Name = "Text_"+shortname
         ...
         finalXml+=getArributeStr("TopMargin",TopMargin)
         finalXml+=getArributeStr("BottomMargin",BottomMargin) 
         finalXml+=getArributeStr("FontSize",FontSize)
         finalXml+=getArributeStr("LabelText",LabelText)
         finalXml+=getArributeStr("ShadowOffsetX",ShadowOffsetX)
         finalXml+=getArributeStr("ShadowOffsetY",ShadowOffsetY)
         finalXml+=getArributeStr("HorizontalAlignmentType",HorizontalAlignmentType)
         finalXml+=getArributeStr("VerticalAlignmentType",VerticalAlignmentType)
         finalXml+=getArributeStr("IsCustomSize","True")
         finalXml+=getArributeStr("ctype",ctype)

         finalXml+= ">\n"
         finalXml+=spaceStr +  getFormatStr("Size",{ X:sizeX,Y:sizeY})
         finalXml+=spaceStr +  getFormatStr("AnchorPoint",{ScaleX:AnchorPointX, ScaleY : AnchorPointY})
         finalXml+=spaceStr +  getFormatStr("Position",{X:PositionX,Y:PositionY})
         finalXml+=spaceStr +  getFormatStr("Scale",{ScaleX:defaultScaleX,ScaleY: defaultScaleY})
         finalXml+=spaceStr +  getFormatStr("CColor",{A:"255",R:ColorR,G:ColorG,B:ColorB})
         finalXml+=spaceStr +  getFormatStr("PrePosition",{X:"0.0",Y:"0.0"})
         finalXml+=spaceStr +  getFormatStr("PreSize",{X:"0.0000", Y:"0.0000"})
         finalXml+=spaceStr +  getFormatStr("OutlineColor",{A:"255",R:"255",G:"0",B:"255"})
         finalXml+=spaceStr +  getFormatStr("ShadowColor",{A:"255",R:"110",G:"110",B:"110"})
    ...
    function genXml(a)
    {
        ...
        try
        {
            if((a.type=="SimpleSprite" || a.type=="Button"  ) && !a.children)
            {
                a.children=deepCopy(symbolLib[a.libName]["children"])           
            }
        }
        catch(e)
        {}
        ...
        //缩进
        var spaceStr = ""
        for(var tabi=1;tabi<=xmlTab;tabi++)
        {
           spaceStr+="    "
        }
        var retContent 
        if(a.type=='Button')
        	retContent= getButtonXml(a,xmlTab)
        else
        {
      
                retContent=getChildRenXml(a,xmlTab)

        }
        if (retContent != -1)  
        {
            childrenXml+=spaceStr + "<AbstractNodeData ";
            childrenXml+=retContent
            if (a.children != null)
            {
                if(a.type=="Button")
                {
                    var beforeadded=false;
                    var endaddeded=false;
                    for(var i in a.children)
                    {
                        var child=a.children[i]
                         if(child.instanceName!="down" && child.instanceName!="normal" )
                         {
                              if(!beforeadded){childrenXml+=spaceStr + "    <Children>\n";beforeadded=true;}
                              xmlTab++;
                              xmlTab++;
                              genXml(child)
                              xmlTab--;
                              xmlTab--;
                               if(!endaddeded){childrenXml+=spaceStr + "    <\/Children>\n";endaddeded=true}
                         }
                    }
                }   
                else
                {
                        ...
                        if (childrenCount == 1&&a.children[0].type == 'Bitmap')  //取bitmap
                        {
                        } 
                        else
                        {
                            childrenXml+=spaceStr + "    <Children>\n";
                            for(var k in a.children)
                            {
                              xmlTab++;
                              xmlTab++;
                              genXml(a.children[k])
    ...
    function writeJson(dataFile,a)
    {
        ...
        {
            if(pre!="{")
                dataFile.write(cacheW)
            //else if(pre=="}") dataFile.write(",\n")
            else
                dataFile.write("\n")
            for(var tabi=0;tabi<tab;tabi++)
                dataFile.write("\t")
            if((typeof k)!="number")
                dataFile.write("\""+k+"\":");
            if((typeof a[k])=="object")
        ...
        if(pre!="o"&& pre!="}")
            dataFile.write("\n")
        for(var tabi=0;tabi<tab-1;tabi++)
            dataFile.write("\t")
        dataFile.write("}\n");
    ...
    {   
        if (DEBUG==null ||DEBUG == false)
            return
        tab=1
        dataFile = new File(basePath + "/json.txt");
        dataFile.encoding = "UTF8";
        dataFile.open( "w", "TEXT", "????" );
        writeJson(dataFile,a[fileNamePre])
    }
...
function exprotAnimXml()
{
    ...
    var finalXml=getExprotCSDHead();
    genXml(symbolLib[fileNamePre])
    finalXml+=childrenXml
    finalXml+=getExprotCSDEnd()

    var xmlFileUrl =  xmlFolderUrl  +"/"+ fileName.replace(".psd",".csd")
