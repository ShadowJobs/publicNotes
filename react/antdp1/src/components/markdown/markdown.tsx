...
import './markdown.css'
// import MarkdownButton from './markdown-blocks/button'
// import MarkdownForm from './markdown-blocks/form'
...
const Link = ({ node, ...props }: any) => {
  if (node.properties?.href && node.properties.href?.toString().startsWith('abbr')) {
    ...
    onClick={() => {}} title={node.children[0]?.value}>{node.children[0]?.value}</abbr>
  }
  else {
    ...
    >{node.children[0] ? node.children[0]?.value : 'Download'}</a>
  }
}
