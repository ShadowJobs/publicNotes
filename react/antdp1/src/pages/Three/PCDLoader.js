...
const intensity2RGB = (intensity) => {
  ...
  if (intensity <= 51) {
    ...
    intensity -= 153;
    B = 0;
    G = 255 - parseInt((128.0 * intensity) / 51.0 + 0.5);
    R = 255;
  } else {
    intensity -= 204;
    B = 0;
    G = 127 - parseInt((127.0 * intensity) / 51.0 + 0.5);
    R = 255;
  }
    ...
    if (PCDheader.data === 'binary_compressed') {
      ...
      for (let i = 0; i < PCDheader.points; i++) {
        ...
        if (offset.intensity !== undefined) {
          const _obj_intensity = dataview.getUint8(
            PCDheader.points * offset.intensity + PCDheader.size[4] * i,
          );
          const rgb = intensity2RGB(_obj_intensity);
          color.push(rgb[2], rgb[1], rgb[0]);
        }
